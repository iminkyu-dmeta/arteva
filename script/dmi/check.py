...
class DMIParse:
    ...
    def dmidecode_parse(self, buffer):  # noqa: C901
        ...
        for record in split_output:
            ...
            data[dmi_handle] = {}
            data[dmi_handle]["Handle"] = handle_data[0]
...
class NMIIParse:
    """
    nmcl information parsing object
    requires nmcli output as input string
    """

    def __init__(self, nmcli_output, default="n/a"):
        self.default = default
        self.data = self.nmcli_parse(nmcli_output)

    def get(self, interface_id=None):
        exist = interface_id in self.data
        if exist:
            result = self.data[interface_id]
            return result
        else:
            return self.data

    def network_status(self, interface_id=None):
        interface_status = {}
        if interface_id:
            exist = interface_id in self.data
            if exist:
                result = self.data[interface_id]
                device = result["GENERAL.DEVICE"]
                if result["GENERAL.TYPE"] == "ethernet" or result["GENERAL.TYPE"] == "bond":
                    state = result["GENERAL.STATE"]
                    interface_status[device] = state

                return interface_status 

        else:
            for i,d in self.data.items():
                device = d["GENERAL.DEVICE"]
                if d["GENERAL.TYPE"] == "ethernet" or d["GENERAL.TYPE"] == "bond":
                    state = d["GENERAL.STATE"]
                    interface_status[device] = state
                
            return interface_status 

    device_re = re.compile("^GENERAL.DEVICE:")

    def nmcli_parse(self, buffer):  
        data = {}
        #  Each record is separated by double newlines
        split_output = buffer.split("\n\n")
        ...
        for record in split_output:
            ...
            if self.device_re.search(record_element[0]):
                device = record_element[0].split(':',maxsplit=1)
                device_field = device[0]
                device_name = device[1].strip()
                
                data[device_name] = {}
                for r in record_element:
                    field = r.split(':',maxsplit=1)
                    data[device_name][field[0]] = field[1].strip()

        return data


class NMCLI(NMIIParse):
    """Network Interface status """

    def __init__(self, command="nmcli,device,show"):
        self.nmcli = command.split(',')
        raw = self._run()
        super().__init__(raw)

    def _run(self):
        with subprocess.Popen(self.nmcli, stderr=subprocess.STDOUT, stdout=subprocess.PIPE) as proc:
            stdout, _ = proc.communicate()
            if proc.returncode > 0:
                raise RuntimeError("{} failed with an error:\n{}".format(self.nmcli, stdout.decode()))
            else:
                return stdout.decode()

def device_show(data):
...
## connected, disconnected, unmanaged
def check_status(dcode, nif):
    p = DMIDecode()
    print("Power Supply")
    if p.get(dcode):
        for i in p.get(dcode):
            for k, v in i.items():
                print("{}: {}".format(k,v))
    else:
        print("{} Not Found".format("Power Supply"))

    print()

    n = NMCLI()
    print("Network Interface ")
    device_show(n.get(nif))
    for k, v in n.network_status(nif).items():
        print("{}: {}".format(k, v))
...
def main():
    ...
    check_status(POWER_SUPPLY, NETWORK_IF)
